...
<template>
   ...
   <div
      id="jumbotron"
      v-if="store.arrayMovies.length > 0 || store.arraySeries.length > 0"
   >
      <img
         :src="this.path + `${store.objectActive.backdrop_path}`"
         :alt="`${store.objectActive.original_title}`"
      />
      <div class="my-container d-flex align-items-center text-white">
         <div id="descriptions">
            <h1 v-if="store.objectActive.title">
               {{ store.objectActive.title }}
            </h1>
            ...
            <div class="cast">
               ...
               <ul class="d-flex p-0">
                  ...
                  <li
                     v-for="(item, index) in store.arrayCast"
                     :key="index"
                     class="small-text"
                  >
                     {{ item.name }}
            ...
            <div class="genres">
               <ul class="d-flex p-0">
                  ...
                  <li v-for="(item, index) in store.arrayGenres" :key="index">
                     {{ item.name }}
                  </li>
               </ul>
            </div>
